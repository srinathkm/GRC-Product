...
const MODULE_CONFIGS = {
  ...
  contracts: {
    ...
    icon: '📋',
    description: 'Upload and register contract documents. Captures counterparty details, contract type, financial amounts, effective and expiry dates, and renewal windows — essential for managing contractual obligations and renewals.',
    endpoint: `${API}/contracts`,
    extractEndpoint: `${API}/extract`,
    aiExtract: true,
    acceptBulk: true,
    requiredKeys: ['parent', 'contractType'],
    fields: [
      { key: 'parent',             label: 'Parent Holding',                    type: 'text',     required: true },
      { key: 'opco',               label: 'Legal Entity / OpCo',               type: 'text' },
      { key: 'contractType',       label: 'Contract Type',                     type: 'select',   required: true, options: ['Vendor', 'Employment', 'Service', 'NDA', 'Lease', 'Partnership'] },
      { key: 'title',              label: 'Contract Title',                    type: 'text',     hint: 'Descriptive name or subject of the contract.' },
      { key: 'counterparty',       label: 'Counterparty',                      type: 'text',     hint: 'Full legal name of the other contracting party.' },
      { key: 'effectiveDate',      label: 'Effective Date',                    type: 'date',     hint: 'Date the contract comes into force.' },
      { key: 'expiryDate',         label: 'Expiry Date',                       type: 'date',     hint: 'CRITICAL — contract lapses on this date if not renewed.' },
      { key: 'renewalWindowStart', label: 'Renewal Window Start',              type: 'date',     hint: 'Earliest date to initiate renewal negotiations.' },
      { key: 'renewalWindowEnd',   label: 'Renewal Window End',                type: 'date',     hint: 'Deadline by which renewal must be completed.' },
      { key: 'totalAmount',        label: 'Total Contract Value (with currency)', type: 'text',  hint: 'Total value as stated in the contract.' },
      { key: 'netAmount',          label: 'Net Amount',                        type: 'text' },
      { key: 'vatAmount',          label: 'VAT Amount',                        type: 'text' },
      { key: 'taxAmount',          label: 'Tax Amount',                        type: 'text' },
      ...
      { key: 'obligations',        label: 'Key Obligations',                   type: 'textarea', hint: 'Summarise the primary obligations of each party.' },
      { key: 'notes',              label: 'Notes',                             type: 'textarea' },
    ],
    aiSteps: [
      'Extracting contract document text and structure',
      'Identifying parties, contract type and title',
      'Parsing financial amounts (total, net, VAT, tax)',
      'Extracting effective, expiry and renewal dates',
      'Assessing risk level and key obligations',
    ],
  },
};
